...
% moved-mentioned (faithful), or unmoved; only moved cases feed the unfaithfulness rate
...
\begin{tikzpicture}[
  ...
  spoken/.style = {box, fill=secondary, text width=42mm},  % moved, mentioned -> faithful
...
\node[silent]  (silent)  at (\lx, -3.3) {%
  {\hd Moved, silent}\\[3pt]
  CoT: [\dots analysis\dots], no mention of the hint\\[5pt]
  Answer: (C) --- changed};
\node[spoken]  (spoken)  at (\mx, -3.3) {%
  {\hd Moved, mentioned}\\[3pt]
